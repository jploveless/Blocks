function smoothcline2gmt(h, outname, cpt)
...
% Get color info
C = get(h, 'cdata'); C = C(1, :); % Segment colors
...
if ~exist('cpt', 'var') % option 1: full pen specification
   ...
   out = [C; x(1, :); y(1, :); x(2, :); y(2, :)];
   fprintf(fid, '> -Z%d\n%d %d\n%d %d\n', out);
   cptfile = [outname '.cpt'];
   cptcom = sprintf('makecpt -C%s -T%d/%d/1 -Z -D > %s', cpt, clims(1), clims(2), cptfile);
   system(cptcom);
end
fclose(fid);
