function AlignAllFields(direc, method, target, varargin)
%
% ALIGNALLFIELDS automates the alignment of an arbitrary number of velocity fields.
%
%  ALIGNALLFIELDS(DIR, METHOD, TARGET) finds all .sta.data files within the directory
...
%
%  ALIGNALLFIELDS(DIR, METHOD, TARGET, PARAM, FUZZ, MINSTA) also accepts an argument that
...
for i = 1:nfiles
   ...
   G{i}                             = zeros(3*nStations(i), nparam);
   for k = 1:nStations(i)
      rowIdx                        = (k-1)*3+1;
      colIdx                        = 1;
      ...
      G{i}(rowIdx:rowIdx+2,colIdx:colIdx+nparam-1) = [R eye(nparam-size(R, 2))];
   end
   G{i}(3:3:end, :)                 = [];
   G{i}                             = G{i}(:, 1:somega);
...
legend(ht, names, 'interpreter', 'none')
...
runCommand                    = sprintf('AlignAllFields(\''%s\'', %g, %g, %g, %g, %g);', direc, method, target, nparam, fuzz, minsta);
save(matname, 'names', 'sumnStations', 'paths', 'nOverlap', 'meanRes', 'runCommand', '-mat');