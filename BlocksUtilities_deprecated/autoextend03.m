...
[Hanglon, Hanglat] = deal(hanglon, hanglat);
...
%   if validhang(i) == 1 % check to see if this point has been handled yet (by a previous extension)
      ...
      if mxodr >= 2
         ...
         if ~ismember([s.lon2(iidx(in)), s.lat2(iidx(in))], [Hanglon Hanglat], 'rows')
            ...
            % Check to see if any hanging point belongs to the split segment; reassign sidx
            [tf, loc] = ismember(iidx(in), sidx);
            if tf
               sidx(loc(ep2(loc))) = length(s.lon1);
            end
            ds = [ds; iidx(in)];
         end
         if ~ismember([s.lon1(iidx(in)), s.lat1(iidx(in))], [Hanglon Hanglat], 'rows')
            ...
            [tf, loc] = ismember(iidx(in), sidx);
            if tf
               sidx(loc(ep1(loc))) = length(s.lon1);
            end
            ds = [ds; iidx(in)];
         end
%         keyboard
      ...
      if mmax > hmax
         ...
         % Check to see if any hanging point belongs to the split segment; reassign sidx
         [tf, loc] = ismember(iidx(midx), sidx);
         if tf
            sidx(loc(ep1(loc))) = length(s.lon1);
            sidx(loc(ep2(loc))) = length(s.lon1) - 1;
         end
         ds = [ds; iidx(midx)];
      else
         if hmax > tscore
