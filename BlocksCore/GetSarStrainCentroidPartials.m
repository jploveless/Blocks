function [G, strainBlockIdx, cLon, cLat]    = GetStrainCentroidPartials(Block, Station, Segment, look_vec, toDelete)
...
%       [lat0 lon0]                           = meanm(latVec(:), lonVec(:));
      ...
      lat0(lat0<0)                          = -90 - lat0(lat0<0);      
      lat0                                  = deg2rad(lat0);
      G(iStation, firstCol(iStation):lastCol(iStation)) = dot(repmat(look_vec(:), 1, 3),...
                                                          [R*(Station.lon(iStation)-lon0).*sin(lat0) R*(Station.lat(iStation)-lat0) 0;...
                                                           0 R*(Station.lon(iStation)-lon0).*sin(lat0) R*(Station.lat(iStation)-lat0); 0 0 0]');
   end
end
