function [Gu, Ge, tz, Patches, Station] = GetTriCombinedPartials(Patches, Station, op)
...
if nPatches > 0
   ...
   parfor (iPatches = 1:nPatches)
      ...
      if sum(op(:, 1)) > 0
         ...
         [uxs uys uzs...
          uxd uyd uzd...
          uxt uyt uzt]                         = tri_dislz_partials([p.px1, p.px2, p.px3], [p.py1, p.py2, p.py3], abs([p.z1, p.z2, p.z3]), s.tpx(op(:, 1)), s.tpy(op(:, 1)), abs(s.z(op(:, 1))), 0.25);
      ...
      if sum(op(:, 2)) > 0
         [uxxs uyys uzzs uxys uxzs uyzs...
          ...
          uxxt uyyt uzzt uxyt uxzt uyzt]       = tri_strain_fast_partials([p.px1, p.px2, p.px3], [p.py1, p.py2, p.py3], abs([p.z1, p.z2, p.z3]), s.tpx(op(:, 2)), s.tpy(op(:, 2)), abs(s.z(op(:, 2))), 0.25);
