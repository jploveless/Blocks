function [G, strainBlockIdx, cLon, cLat]    = GetStrainReferencePartials(Block, Station, Segment, lon0Vec, lat0Vec, look_vec)
...
%       lon0                                  = mean(lonVec(:));
      ...
      lat0(lat0<0)                          = -90 - lat0(lat0<0);      
      lat0                                  = deg2rad(lat0);
      G(2*iStation-1:2*iStation, firstCol(iStation):lastCol(iStation)) = [R*(Station.lon(iStation)-lon0).*sin(lat0) R*(Station.lat(iStation)-lat0) 0;...
