function [ux, uy, uz, normVec]       = tri_dislz(x, y, z, sx, sy, sz, ss, ds, ts, pr);
%
% tri_disl.m
%
...
for iTri = 1:3
   ...
   % Slip components following CalcTriDisps.m
...
%inPolyIdx                       = intriangle(sx, sy, x, y);
%t                               = LinePlaneIntersect(x, y, z, normVec, sx, sy, sz);
%underIdx                        = sum([t > 0 & t < 1, inPolyIdx], 2) > 1;
%
%% Apply static offset to the points that lie underneath the current triangle
%ux                    = ux - underIdx*slipVec(1);
%uy                    = uy - underIdx*slipVec(2);
%uz                    = uz - underIdx*slipVec(3);
%% ux(underIdx)                    = ux(underIdx) - slipVec(1);
%% uy(underIdx)                    = uy(underIdx) - slipVec(2);
%% uz(underIdx)                    = uz(underIdx) - slipVec(3);
%
%
...
%numerator                       = dot(repmat(normVec', size(sx)), repmat([x(1), y(1), z(1)], size(sx))-[sx, sy, sz], 2);
%denominator                     = dot(repmat(normVec', size(sx)), [sx, sy, 0*sx]-[sx, sy, sz], 2);
