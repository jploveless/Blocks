function WriteOutput(Segment, Patches, Station, Block, Command, Model, Strikes, ep)
...
WriteStation(sprintf('.%s%s%seTri.sta.data', filesep, runName, filesep), Station.lon, Station.lat, Model.eastEmbVel, Model.northEmbVel, zeros(size(Station.eastSig)), zeros(size(Station.northSig)), Station.corr, Station.blockLabel, Station.tog, Station.name);
...
WritePatches(sprintf('.%s%s%sMod.patch', filesep, runName, filesep), Patches.c, Patches.v, Model.trislipS, Model.trislipD, Model.trislipT, Model.trislipSSig, Model.trislipDSig, Model.trislipTSig, Strikes, Model.trislipSBlock, Model.trislipDBlock, Model.trislipTBlock);		

% Write out embedded triangular slips
if ~isempty(ep.c)
   WritePatches(sprintf('.%s%s%seMod.patch', filesep, runName, filesep), ep.c, ep.v, Model.etrislipS, Model.etrislipD, Model.etrislipT, Model.etrislipSSig, Model.etrislipDSig, Model.etrislipTSig, ep.strike, zeros(ep.nEl, 1), zeros(ep.nEl, 1), zeros(ep.nEl, 1));
end
