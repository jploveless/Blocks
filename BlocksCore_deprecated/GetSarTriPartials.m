...
if nPatches > 0;
   parfor (iPatches = 1:nPatches)
      ...
      ts(iPatches)                             = baz;
      ulos                                     = dot(repmat(look_vec(:), 1, nStations), [ux'; uy'; uz']);
      v1{iPatches}                             = reshape(ulos', nStations, 1);
      ...
      if abs(abs(dip) - 90) > 2.5;
         ...
         ulos                                  = dot(repmat(look_vec(:), 1, nStations), [ux'; uy'; uz']);
         v2{iPatches}                          = reshape(ulos', nStations, 1);
         v3{iPatches}                          = zeros(nStations, 1);
         tz(iPatches)                          = 2;
      else
         ...
         ulos                                  = dot(repmat(look_vec(:), 1, nStations), [ux'; uy'; uz']); 
         v3{iPatches}                          = reshape(ulos', nStations, 1);
         v2{iPatches}                          = zeros(nStations, 1);
         tz(iPatches)                          = 3;
      end   
   end
