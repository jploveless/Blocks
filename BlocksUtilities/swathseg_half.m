function [sx, sy, seg] = swathseg_half(seg, d)
...
for i = 2:nseg
   ...
   paz1 = maz + 90;
   paz2 = maz - 90;
   [sy(idx1), sx(idx1)] = deal(seg.lat1(i), seg.lon1(i));
   if dr == 1
      [sy(idx2), sx(idx2)] = reckon(seg.lat1(i), seg.lon1(i), d, paz1);
   elseif dr == -1
...
% Treat endpoints
[sy(1), sx(1)] = deal(seg.lat1(1), seg.lon1(1));
[sy(end), sx(end)] = reckon(seg.lat1(1), seg.lon1(1), d, az(1)+dr*90);
[sy(nseg+1), sx(nseg+1)] = deal(seg.lat2(nseg), seg.lon2(nseg));
[sy(nseg+2), sx(nseg+2)] = reckon(seg.lat2(nseg), seg.lon2(nseg), d, az(nseg)+dr*90);
