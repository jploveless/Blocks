...
%   if validhang(i) == 1 % check to see if this point has been handled yet (by a previous extension)
      ...
      % Get rid of the inersection point that is the hanging point's segment's other endpoint
      depa = abs(ilat(in) - hanglat(i));
      depo = abs(ilon(in) - hanglon(i));
      oep = (depa + depo) < 1e-3;
      in = in(oep);
      od1 = distance(ilat(in), ilon(in), S.lat1(sidx(i)), S.lon1(sidx(i)));
      od2 = distance(ilat(in), ilon(in), S.lat2(sidx(i)), S.lon2(sidx(i)));
      odr = od1./od2; % ratio of overlap lengths
      % If distance from intersection to endpoint 1 is twice (or more) the length of the
      % distance to endpoint 2, reassign endpoint 2 to be the intersection
      if odr >= 2
      ...
      iidx = iidx(~inseg1 & inseg2);
      % Get rid of the intersection point that is the hanging point's segment's other endpoint
      depa = abs(ilat - (ep2(i)*S.lat1(i) + ep1(i)*S.lat2(i)));
      depo = abs(ilon - (ep2(i)*S.lon1(i) + ep1(i)*S.lon2(i)));
      oep = (depa + depo) > 1e-6;
      ilat = ilat(oep);
      ilon = ilon(oep);
      iidx = iidx(oep);
